...
def get_students_bulk(conn: sqlite3.Connection, reg_nos: List[str]):
    """
    Takes a list of Reg Nos and returns a dict: 
    {'2023CA001': {'name': 'Rahul', 'branch': 'MCA'}, ...}
    """
    if not reg_nos:
        return {}
    
    placeholders = ','.join(['?'] * len(reg_nos))
    query = f"SELECT reg_no, name, email, year, section FROM students WHERE reg_no IN ({placeholders})"
    
    cursor = conn.cursor()
    cursor.execute(query, list(reg_nos))
...
def get_all_clubs_logic(clubs_file, conn, current_user, category=None):
    ...
    if category:
        clubs_data = [c for c in clubs_data if c.get("category", "").lower() == category.lower()]
    ...
    for club in clubs_data:
        ...
        if is_coordinator: club["user_status"] = "coordinator"
        elif is_member: club["user_status"] = "member"
        else: club["user_status"] = "none"
...
def update_club_logic(clubs_file, club_id, payload: ClubUpdateSchema, current_user):
    ...
    # Permission check: Only coordinators or professors can update
    ...
    if payload.short_description: club["short_description"] = payload.short_description
    if payload.full_description: club["full_description"] = payload.full_description
    if payload.category: club["category"] = payload.category
    if payload.tags: club["tags"] = payload.tags
...
def recommend_clubs_ai(clubs_file, request: RecommendRequest):
    """
    Uses Google Gemini to match student interests with Club data.
    """
    ...
    # 1. Prepare minimal club data to save token context
    club_summaries = []
    for c in clubs_data:
    ...
    Task:
    1. Analyze the student's input and identifying keywords.
    2. Select the top 3 clubs that best align with their profile.
    3. If the input is vague (e.g., "I like fun"), infer general social clubs (Cultural/Sports).
    4. Provide a 'match_score' (0-100) based on relevance.
    5. Write a 'reason' for each. The reason MUST be conversational, encouraging, and specific. 
       - Address the student directly (e.g., "Since you enjoy debate...").
       - Explain *why* this club fits their specific query.
       - Keep the reason under 2 sentences.
    ...
    try:
        ...
        if not response.parsed:
            # Fallback if AI refuses to answer or returns empty
            return []

        recommendations = response.parsed.recommendations
        
        # 3. Merge AI results with full Club Data
        final_results = []
        for rec in recommendations:
            original_club = next((c for c in clubs_data if c["id"] == rec.id), None)
            if original_club:
                # Add AI metadata to the club object for frontend display
